...
const Movies = () => {
  ...
  const [valuegene,SetValueGene] = use
  ...
  useEffect(() => {
    getupcoming.then((response) => {
      ...
      // setGeneres([...generes, response.data.results[random].genre_ids]);
      ...
      getgenre.then((response) => {
        ...
        // generes.forEach((item, index) => {
        //   const resutl = dataGene.map((ind) => ind.id === 18);
        //   setGeneres([...generes, resutl]);
        //   console.log(resutl);
        // });
        // const result = dataGene
        // const result = dataGene.find((e) => e.id === 18);
        setGeneres([...generes, dataGene]);
      });
    });
  }, []);
  const test = () => {
    generes[0].forEach((itemm) => {
      const result = generes[0].find((item) => item.id === 18);
      console.log(result);
    });
    // var result = generes[0].find((item) => item.id === 18);
    // console.log(result);
    // console.log(generes[0]);
  };
  return (
    <div>
